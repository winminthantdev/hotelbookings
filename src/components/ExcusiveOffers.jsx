...
const ExcusiveOffers = () => {
  return (
    <div className='flex flex-col items-center px-6 md:px-16 lg:px-24 xl:px-32 pt-20 pb-30'>
        ...
        <Title align='left' title='Excusive Offers' subTitle='Take advantage of our limited-time offers and specical packages ot enhance your stay and create unforgettable memories.' />
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12'>
            {exclusiveOffers.map((item)=>(
                <div key={item._id} className='group relative flex flex-col items-start justify-between gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-cover bg-center' style={{backgroundImage: `url(${item.image})`}}>
                    <p className='absolute top-4 left-4 text-xs bg-white text-gray-800 font-medium rounded-full px-3 py-1'>{item.priceOff}% OFF</p>
                    <div>
